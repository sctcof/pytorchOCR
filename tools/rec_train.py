...
def ModelTrain(train_data_loader,LabelConverter, model,criterion,optimizer, train_log,loss_dict, config, epoch):
    for batch_idx, ( imgs,labels) in enumerate(train_data_loader[0]):
        # print(batch_idx)
        pre_batch = {}
        gt_batch = {}
        ...
        if (batch_idx % config['base']['show_step'] == 0):
            log = '({}/{}/{}/{}) | ' \
                .format(epoch, config['base']['n_epoch'], batch_idx, len(train_data_loader[0]))
...
def ModelEval(val_data_loader,LabelConverter, model,criterion,train_log,loss_dict,config):
    bar = tqdm(total=len(val_data_loader[0]))
    val_loss = AverageMeter()
    n_correct = AverageMeter()
    for batch_idx, (imgs, labels) in enumerate(val_data_loader[0]):
        bar.update(1)
...
def TrainValProgram(args):
    ...
    # val_dataset = create_module(config['valload']['function'])(config)
    val_dataset = create_module(config['trainload']['function'])(config,'val')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', help='config file path',default='../config/rec_CRNN_mobilev3_small_english_all.yaml')
    parser.add_argument('--log_str', help='log title',default='train_rec_log')
