...
"""
@author:fxw
@file: transform_img.py，图片转换脚本
@time: 2020/08/11
"""
##imgaug数据增强神器：第四章 增强关键点/界标
##https://blog.csdn.net/lly1122334/article/details/88874502
import cv2
import numpy as np
...
class Random_Augment():
    ...
    def random_rotate(self, img, polys, random_range=[-10,10]):
        ...
        aug_bin = aug_img.Sequential([aug_img.Affine(rotate=angle)]) ##imgaug.augmenters将对图片的操作进行汇总，最后统一运行
    ...
    ##对图片尺寸进行随机变化处理,依据最小尺寸min_size，对图片大小进行调整
    def random_scale(self, img, polys, min_size):
        polys, len_max = solve_polys(polys) #多边形边框格式转化
        h, w = img.shape[0:2]
        new_polys = []
        for poly in polys: ##对每个多边形进行处理
            poly = np.asarray(poly)
            poly = poly / ([w * 1.0, h * 1.0] * len_max) ##对多边形对每个点坐标进行尺度缩放
            new_polys.append(poly)
        ...
        new_polys = np.reshape(new_polys * ([img.shape[1], img.shape[0]] * len_max),
                               (new_polys.shape[0], polys.shape[1] // 2, 2)) ##依据新图片对尺寸，对边框进行缩放
        return img, new_polys
